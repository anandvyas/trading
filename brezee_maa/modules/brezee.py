import os
import logging
import urllib
import pandas as pd
import time
from datetime import date, datetime, timedelta

from dotenv import load_dotenv, set_key

from pyotp import TOTP
import asyncio
from pyppeteer import launch
from breeze_connect import BreezeConnect

from modules.utility import Utility

load_dotenv()
logging.basicConfig(format='%(asctime)s - %(message)s',level=logging.DEBUG)

api_key =  os.environ.get("BREEZE_API_KEY")
secret_key = os.environ.get("BREEZE_SECRET_KEY")
session_token = os.environ.get("BREEZE_SESSION")


class Breeze():

    # constructor
    def __init__(self, salt='SlTKeYOpHygTYkP3') -> None:
        self.u = Utility()
        self.user = None
        self.breeze = BreezeConnect(api_key=api_key)
        self._login()
    
    def _login(self):
        try:
            self.breeze.generate_session(api_secret=secret_key,session_token=session_token)
            logging.info(f"ICICI Breeze login successfully...")
            self.user = self.breeze.get_customer_details(api_session=session_token)["Success"]
            logging.info(f"Welcome {self.user['idirect_user_name']}")

        except Exception as e:
            logging.debug(f"ICICI Breeze session not valid...")
            logging.debug(f"Initiate new session request")
            asyncio.get_event_loop().run_until_complete(self._getSession())

    def _isMarketActive(self):
        today = datetime.today().strftime("%d-%b-%Y")
        if today == self.user["exg_trade_date"]["NSE"]:
            return True
        else:
            logging.info(f"Today is holiday, enjoy you day !!!")
            return False
    
    def _isSegmentActive(self, segment = "Derivatives"):
        if segment.title() not in ['Trading','Equity','Derivatives','Currency']:
            return False
        else:
            if self.user["segments_allowed"][segment.title()] == 'Y':
                return True
            else:
                return False

    def _getLTP(self, stock_code:str):
        try:
            return self.breeze.get_quotes(stock_code=stock_code.upper(), exchange_code="NSE")['Success'][0]['ltp']
        except Exception as e:
            logging.error(str(e))

    def _getATMStrikePrice(self, symbol:str):
        try:
            ltp = self._getLTP(stock_code=symbol.upper())
            base = self.u._getStrikeDiff(symbol.upper())
            return self.u._getATMStrikePrice(ltp, base)
        except Exception as e:
            logging.error(str(e))

    def _getStrikes(self, symbol:str, count = 10):
        try:
            base = self.u._getStrikeDiff(symbol.upper())
            atm = self._getATMStrikePrice(symbol.upper())
            start = atm - (base * count)
            end = atm + (base * count) + base
            return list(range(start, end, base))
        except Exception as e:
            logging.error(str(e))

    def _getExpiries(self, symbol:str):
        expiries = []
        try:
            sp = self._getATMStrikePrice(symbol.upper())
            oc = self.breeze.get_option_chain_quotes(stock_code=symbol.upper(), exchange_code="NFO", product_type="options", right="call", strike_price=sp)['Success']
            for i in oc:
                expiries.append(i["expiry_date"])
            return expiries
        except Exception as e:
            logging.error(str(e))

    def _getCurrentExpiry(self, symbol:str):
        return self._getExpiries(symbol.upper())[0]
    
    def _getNextExpiry(self, symbol:str):
        return self._getExpiries(symbol.upper())[1]
    
    def _getFarExpiry(self, symbol:str):
        return self._getExpiries(symbol.upper())[2]
    
    def _getExpiry(self, stock_code:str, expiry = "next"):
        if expiry not in ["current","next","far"]:
            expiry = "next"
        
        return eval(f"self._get{expiry.title()}Expiry")(stock_code.upper())

    def _getOptionChain(self, stock_code:str, expiry = "next"):
        try:
            expiry = self._getExpiry(stock_code.upper(),expiry)
            allowColumns = ["strike_price","ltp","open_interest","total_quantity_traded","total_buy_qty","total_sell_qty"]
            
            occ = self.breeze.get_option_chain_quotes(stock_code=stock_code.upper(),
                                                 exchange_code="NFO",
                                                 product_type="options",
                                                 expiry_date=expiry,
                                                 right="call")["Success"]
            
            df = pd.DataFrame(occ,columns=allowColumns)
            df.index = df['strike_price']
            df = df.add_prefix("call_")
            
            ocp = self.breeze.get_option_chain_quotes(stock_code=stock_code.upper(),
                                                 exchange_code="NFO",
                                                 product_type="options",
                                                 expiry_date=expiry,
                                                 right="put")["Success"]
            
            df2 = pd.DataFrame(ocp,columns=allowColumns)
            df2.index = df2['strike_price']
            df2 = df2.add_prefix("put_")
            
            return pd.merge(df, df2, left_index=True, right_index=True)
        except Exception as e:
            logging.error(str(e))

    def _optionPlaceOrder(self, stock_code, strike_price, quantity, 
                          expiry = "next", right = ["call","put"], 
                          action = ["buy","sell"]):
        
        
        exchange_code = "NFO"
        product = "options"
        order_type = "market"
        validity = "day"
        expiry_date = self._getExpiry(stock_code.upper(),expiry)
        validity_date = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ')

    # S - underline, K - strike, T -  time remaining r - intrest rate s - sigma 
        

    def _placeOrder(self, stock_code,exchange_code,product,action,order_type,
                    stoploss,quantity,price,validity,validity_date,expiry_date,
                    right,strike_price):
         
        data =  self.breeze.place_order(
                stock_code=stock_code,
                exchange_code=exchange_code,
                product=product,
                action = action,
                order_type=order_type,
                stoploss=stoploss,
                quantity=quantity,
                price = price,
                validity= validity,
                validity_date = validity_date,
                disclosed_quantity = "0",
                expiry_date = expiry_date,
                right= right,
                strike_price=strike_price)
        response = None
        if(data['Status'] == 200):
            response = data['Success']['message']
            logging.info(f"Success : {response} for {right} with order_id :{data['Success']['order_id']}")
        else:
            response = data['Error']
            logging.error(f"Error : {response} for {right}")
        return(data)
        

    async def _getSession(self):
        logging.debug(f"ICICI Breeze login initiated")
        # login to ICICI Breeze
        url = "https://api.icicidirect.com/apiuser/login?api_key="+urllib.parse.quote_plus(os.environ.get("BREEZE_API_KEY"))
        browserObj = await launch({"headless": False})
        site = await browserObj.newPage()
        await site.goto(url)
        await site.waitFor(1000)

        await site.waitForSelector("#txtuid")
        await site.type('#txtuid', os.environ.get("BREEZE_USER"))
        await site.waitForSelector("#txtPass")
        await site.type('#txtPass', os.environ.get("BREEZE_PASS"))
        checkboxEl = await site.waitForSelector("#chkssTnc")
        await checkboxEl.click()

        # Submit the form
        await site.keyboard.press('Enter')
        await site.waitFor(1000)

        # Manage TOTP
        totp = TOTP(os.environ.get("BREEZE_TOTP"))
        token = totp.now()
        await site.waitForSelector("#hiotp")
        await site.type("#hiotp", token)

        await site.waitForSelector("#Button1")
        await site.click('#Button1')
        await site.waitFor(1000)

        session = site.url #.split("apisession=")[1][:8]
        await site.waitFor(10000)
        await browserObj.close()
        
        return session



